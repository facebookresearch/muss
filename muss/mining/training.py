...
def get_mbart_kwargs(dataset, language, use_access, use_short_name=False):
    ...
    kwargs = {
        ...
        'train_kwargs': add_dicts(
            {'ngpus': 8},
            args_str_to_dict(
                f'''--restore-file {mbart_path}  --skip-invalid-size-inputs-valid-test --memory-efficient-fp16 --arch mbart_large --task translation_from_pretrained_bart  --source-lang {source_lang} --target-lang {target_lang}  --encoder-normalize-before --decoder-normalize-before --criterion label_smoothed_cross_entropy --label-smoothing 0.2  --dataset-impl mmap --optimizer adam --adam-eps 1e-06 --adam-betas '(0.9, 0.98)' --lr-scheduler polynomial_decay --lr 3e-05 --min-lr -1 --warmup-updates 500 --total-num-update 20000 --dropout 0.3 --attention-dropout 0.1  --weight-decay 0.0 --max-tokens 928 --update-freq 16 --log-format simple --log-interval 2 --reset-optimizer --reset-meters --reset-dataloader --reset-lr-scheduler --langs ar_AR,cs_CZ,de_DE,en_XX,es_XX,et_EE,fi_FI,fr_XX,gu_IN,hi_IN,it_IT,ja_XX,kk_KZ,ko_KR,lt_LT,lv_LV,my_MM,ne_NP,nl_XX,ro_RO,ru_RU,si_LK,tr_TR,vi_VN,zh_CN
